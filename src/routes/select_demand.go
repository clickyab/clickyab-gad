...
func (tc *selectController) selectDemandWebAd(c echo.Context, rd *middlewares.RequestData, e *srtb.BidRequest) error {
	...
	filteredAds := selector.Apply(&m, selector.GetAdData(), sel)
	show, ads := tc.makeShow(c, "sync", rd, filteredAds, nil, sizeNumSlice, slotSize, nil, website, false, bidfloors, false, true, config.Config.Clickyab.FloorDiv.Demand, true)
...
func (tc *selectController) getWebDataExchangeFromCtx(c echo.Context, rd *middlewares.RequestData, e *srtb.BidRequest) (*middlewares.RequestData, *srtb.BidRequest, *mr.Website, int64, error) {
	name, userID, err := config.GetSupplier(rd.SupplierKey)
	if err != nil {
...
func (tc selectController) slotSizeWebExchange(imps []srtb.Impression, website mr.Website) (map[string]*slotData, map[string]int, map[string]string, map[string]int64, error) {
	...
	for i := range imps {
		size, err := config.GetSize(fmt.Sprintf("%dx%d", imps[i].Banner.Width, imps[i].Banner.Height))
