...
func (m *Manager) FetchWebsiteByPublicID(publicID string) (*Website, error) {
	var res = Website{}
	key := fmt.Sprintf("website_%s", publicID)
	cc := cache.CreateCacheWrapper(key, &res)
	err := cache.Hit(key, cc)
	if err == nil {
		return &res, nil
	}
	...
	err = cache.Cache(cc, time.Hour, err)
	if err != nil {
		return nil, err
	}
	return &res, nil
...
func (m *Manager) FetchWebsite(ID int64) (*Website, error) {
	var res = Website{}
	key := fmt.Sprintf("website_id_%d", ID)
	cc := cache.CreateCacheWrapper(key, &res)
	err := cache.Hit(key, cc)
	if err == nil {
		return &res, nil
...
func (m *Manager) FetchWebsiteByDomain(domain string) (*Website, error) {
	var res = Website{}
	key := fmt.Sprintf("website_domain_%s", domain)
	cc := cache.CreateCacheWrapper(key, &res)
	err := cache.Hit(key, cc)
	if err == nil {
		return &res, nil
	...
	query := `SELECT * FROM websites WHERE w_domain = ?   LIMIT 1`
