...
run-gad: gad
	sudo setcap cap_net_bind_service=+ep $(BIN)/gad
	$(BIN)/gad
...
lint: $(LINTER)
	...
	$(LINTERCMD) $(ROOT)/src/cache
	$(LINTERCMD) $(ROOT)/src/cmd
	$(LINTERCMD) $(ROOT)/src/config
	$(LINTERCMD) $(ROOT)/src/eav
	$(LINTERCMD) $(ROOT)/src/entity
	$(LINTERCMD) $(ROOT)/src/pool
	$(LINTERCMD) $(ROOT)/src/rtb
