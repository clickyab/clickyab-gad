...
// Slot is the slot in the site or app
type Slot struct {
...
// Country is clients country info
type Country struct {
	Valid bool   `json:"valid"`
	Name  string `json:"name"`
	ISO   string `json:"iso"`
...
// Province is clients province info
type Province struct {
	Valid bool   `json:"valid"`
	Name  string `json:"name"`
}

// LatLon is info about clients location
type LatLon struct {
	Valid bool    `json:"valid"`
	Lat   float64 `json:"lat"`
	Long  float64 `json:"long"`
...
func requestDataRenderer(ctx echo.Context, e RequestDataFromExchange) *RequestData {
	rde := &RequestData{
		SuppliersName: e.Source.Supplier,
		IP:            net.ParseIP(e.IP),
	...
	if e.Platform == "app" {
		rde.fillAppAttr(e.Attributes)
	}
	return rde
}

func (rde *RequestData) fillAppAttr(attr map[string]interface{}) {
	if v, ok := attr["brand"]; ok {
		rde.Brand = v.(string)
	}
	if v, ok := attr["cid"]; ok {
	...
	if v, ok := attr["model"]; ok {
		rde.Model = v.(string)
	}
	if v, ok := attr["operator"]; ok {
		rde.Operator = v.(string)
	}
	if v, ok := attr["os_identity"]; ok {
		rde.OSIdentity = v.(string)
	}
	if v, ok := attr["carrier"]; ok {
